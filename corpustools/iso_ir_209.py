...
decoding_table = (
    u'\u0000'     # 0x00 -> NULL (NUL)
    u'\u0001'     # 0x01 -> START OF HEADING (SOH)
    u'\u0002'     # 0x02 -> START OF TEXT (STX)
    u'\u0003'     # 0x03 -> END OF TEXT (ETX)
    u'\u0004'     # 0x04 -> END OF TRANSMISSION (EOT)
    u'\u0005'     # 0x05 -> ENQUIRY (ENQ)
    u'\u0006'     # 0x06 -> ACKNOWLEDGE (ACK)
    u'\u0007'     # 0x07 -> BELL (BEL)
    u'\u0008'     # 0x08 -> BACKSPACE (BS)
    u'\u0009'     # 0x09 -> CHARACTER TABULATION (HT)
    u'\u000A'     # 0x0a -> LINE FEED (LF)
    u'\u000B'     # 0x0b -> LINE TABULATION (VT)
    u'\u000C'     # 0x0c -> FORM FEED (FF)
    u'\u000D'     # 0x0d -> CARRIAGE RETURN (CR)
    u'\u000E'     # 0x0e -> SHIFT OUT (SO)
    u'\u000F'     # 0x0f -> SHIFT IN (SI)
    u'\u0010'     # 0x10 -> DATALINK ESCAPE (DLE)
    u'\u0011'     # 0x11 -> DEVICE CONTROL ONE (DC1)
    u'\u0012'     # 0x12 -> DEVICE CONTROL TWO (DC2)
    u'\u0013'     # 0x13 -> DEVICE CONTROL THREE (DC3)
    u'\u0014'     # 0x14 -> DEVICE CONTROL FOUR (DC4)
    u'\u0015'     # 0x15 -> NEGATIVE ACKNOWLEDGE (NAK)
    u'\u0016'     # 0x16 -> SYNCHRONOUS IDLE (SYN)
    u'\u0017'     # 0x17 -> END OF TRANSMISSION BLOCK (ETB)
    u'\u0018'     # 0x18 -> CANCEL (CAN)
    u'\u0019'     # 0x19 -> END OF MEDIUM (EM)
    u'\u001A'     # 0x1a -> SUBSTITUTE (SUB)
    u'\u001B'     # 0x1b -> ESCAPE (ESC)
    u'\u001C'     # 0x1c -> FILE SEPARATOR (IS4)
    u'\u001D'     # 0x1d -> GROUP SEPARATOR (IS3)
    u'\u001E'     # 0x1e -> RECORD SEPARATOR (IS2)
    u'\u001F'     # 0x1f -> UNIT SEPARATOR (IS1)
    u'\u0020'     # 0x20 -> SPACE
    ...
    u'\u00BF'     # 0x8a -> INVERTED QUESTION MARK
    u'\u2264'     # 0x8b -> LESS-THAN OR EQUAL TO
    ...
    u'\u2013'     # 0x96 -> EN DASH
    u'\u2014'     # 0x97 -> EM DASH
    u'\u00AE'     # 0x98 -> REGISTERED SIGN
    u'\u2122'     # 0x99 -> TRADE MARK SIGN
    u'\u00A1'     # 0x9a -> INVERTED EXCLAMATION MARK
    u'\u2265'     # 0x9b -> GREATER-THAN OR EQUAL TO
    ...
    u'\u010C'     # 0xa1 -> LATIN CAPITAL LETTER C WITH CARON
    u'\u010D'     # 0xa2 -> LATIN SMALL LETTER C WITH CARON
    u'\u0110'     # 0xa3 -> LATIN CAPITAL LETTER D WITH STROKE
    u'\u0111'     # 0xa4 -> LATIN SMALL LETTER D WITH STROKE
    u'\u01E4'     # 0xa5 -> LATIN CAPITAL LETTER G WITH STROKE
    u'\u01E5'     # 0xa6 -> LATIN SMALL LETTER G WITH STROKE
    u'\u00A7'     # 0xa7 -> SECTION SIGN
    u'\u01E6'     # 0xa8 -> LATIN CAPITAL LETTER G WITH CARON
    u'\u00A9'     # 0xa9 -> COPYRIGHT SIGN
    u'\u01E7'     # 0xaa -> LATIN SMALL LETTER G WITH CARON
    u'\u021E'     # 0xab -> LATIN CAPITAL LETTER H WITH CARON
    u'\u01E8'     # 0xac -> LATIN CAPITAL LETTER K WITH CARON
    u'\u00AD'     # 0xad -> SOFT HYPHEN
    u'\u01E9'     # 0xae -> LATIN SMALL LETTER K WITH CARON
    u'\u014A'     # 0xaf -> LATIN CAPITAL LETTER ENG (Sami)
    u'\u00B0'     # 0xb0 -> DEGREE SIGN
    u'\u014B'     # 0xb1 -> LATIN SMALL LETTER ENG (Sami)
    u'\u0160'     # 0xb2 -> LATIN CAPITAL LETTER S WITH CARON
    u'\u0161'     # 0xb3 -> LATIN SMALL LETTER S WITH CARON
    u'\u00B4'     # 0xb4 -> ACUTE ACCENT
    u'\u0166'     # 0xb5 -> LATIN CAPITAL LETTER T WITH STROKE
    u'\u00B6'     # 0xb6 -> PILCROW SIGN
    u'\u00B7'     # 0xb7 -> MIDDLE DOT
    u'\u0167'     # 0xb8 -> LATIN SMALL LETTER T WITH STROKE
    u'\u017D'     # 0xb9 -> LATIN CAPITAL LETTER Z WITH CARON
    u'\u017E'     # 0xba -> LATIN SMALL LETTER Z WITH CARON
    u'\u021F'     # 0xbb -> LATIN SMALL LETTER H WITH CARON
    u'\u01B7'     # 0xbc -> LATIN CAPITAL LETTER EZH
    u'\u0292'     # 0xbd -> LATIN SMALL LETTER EZH
    u'\u01EE'     # 0xbe -> LATIN CAPITAL LETTER EZH WITH CARON
    u'\u01EF'     # 0xbf -> LATIN SMALL LETTER EZH WITH CARON
    ...
    u'\u00D0'     # 0xd0 -> LATIN CAPITAL LETTER ETH (Icelandic)
    ...
    u'\u00DE'     # 0xde -> LATIN CAPITAL LETTER THORN (Icelandic)
    u'\u00DF'     # 0xdf -> LATIN SMALL LETTER SHARP S (German)
    ...
    u'\u00F0'     # 0xf0 -> LATIN SMALL LETTER ETH (Icelandic)
    ...
    u'\u00FE'     # 0xfe -> LATIN SMALL LETTER THORN (Icelandic)
