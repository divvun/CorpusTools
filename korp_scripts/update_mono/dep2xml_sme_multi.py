...
def process_file(current_file):
    ...
    if current_file.endswith('.xml'):
        ...
        xml_tree.write(os.path.join(current_out_dir_path,str(current_file.split("/")[-1])),
...
def split_cohort(analysis, current_lang):
    ...
    for extra_tag in ['<cohort-with-dynamic-compound>','<ext>','<cs>','<hab>','<loc>','<gen>','<ctjHead>']:
    ...
    for current_sentence in [x for x in re.split('\n\n', _analysis) if x != '']:
        ...
        for current_cohort in [x for x in re.split('^"<|\n"<', current_sentence) if x != '']:
            ...
            # filter all Err- tags from the string
            used_analysis = re.sub('Err/[^\s]+\s','',used_analysis)
            #used_analysis = re.sub('Sem/[^\s]+\s','',used_analysis)
            # group all Sem- tags according to predefined categories
            ...
            #print("used_analysis=",used_analysis)
            parts = re.compile("(_∞_\w+\s?|_∞_\?\s?|_∞_\<ehead>\s?|_∞_#|_∞_\<mv>\s?)").split(used_analysis, 1)
...
def generate_lemma(in_string, c_lang):
    ...
    generation_cmd = ' | hfst-lookup -q ' + langs_dir + _current_lang + '/src/generator-gt-norm.hfstol'
